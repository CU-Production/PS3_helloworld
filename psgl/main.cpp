...
#define SAMPLE_NAME "Basic Cg Example"
...
gfxObject Object;			//a simple object to render
...
void sampleInit() 
{
  ...
  Object.setMesh(CUBEVCOUNT,cubeST,cubeNorm,cubeVert);
  Object.mTexture = gfxCreateCheckerTexture(8,0x000000FF,0xFF0000FF);
  Object.mRotD = Vector3(0.003f,0.004f,0.002f);    //this simply makes the object spin
...
void sampleUpdateMatrices()
{
  Object.update();		//updates the objects orientation and local to world matrix
...
void sampleRender()
{
  ...
  //apply the local to world matrix to get the complete  model to view matrix set as current
  glMultMatrixf((GLfloat*)&Object.mLocalToWorld);		//load the local to world matrix


  cgGLEnableProfile(cgGLGetLatestProfile(CG_GL_VERTEX));	//enable the shaders
  cgGLEnableProfile(cgGLGetLatestProfile(CG_GL_FRAGMENT));
  
  //set the uniforma paramters in the cg programs
  ...
  cgGLSetTextureParameter (DiffuseMap_cgParam, Object.mTexture);   gfxCheckCgError (__LINE__);
  cgGLEnableTextureParameter (DiffuseMap_cgParam);                 gfxCheckCgError (__LINE__);

  Object.render();		//draws the object
